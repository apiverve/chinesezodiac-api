...
class ChinesezodiacValidationException implements Exception {
  final List<String> errors;

  ChinesezodiacValidationException(this.errors);

  @override
  String toString() => 'ChinesezodiacValidationException: ${errors.join("; ")}';
}
...
/// Chinese Zodiac API client.
///
/// For more information, visit: https://apiverve.com/marketplace/chinesezodiac?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [date] (required) - The date in YYYY-MM-DD format (years 1900-2100) [format: date]
class ChinesezodiacClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'date': ValidationRule(type: 'string', required: true, format: 'date'),
  };

  ChinesezodiacClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/chinesezodiac',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [ChinesezodiacValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw ChinesezodiacValidationException(errors);
    }
  }

  /// Execute a request to the Chinese Zodiac API.
  ///
  /// Parameters are validated before sending the request.
  Future<ChinesezodiacResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw ChinesezodiacException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ChinesezodiacResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw ChinesezodiacException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw ChinesezodiacException('Resource not found');
      } else {
        throw ChinesezodiacException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is ChinesezodiacException) rethrow;
      throw ChinesezodiacException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Chinese Zodiac API client.
class ChinesezodiacException implements Exception {
  final String message;

  ChinesezodiacException(this.message);

  @override
  String toString() => 'ChinesezodiacException: $message';
}
